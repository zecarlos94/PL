...
void insertAVL(AVL a, char* author) {
	...
	else if(strcmp(author, a->author) < 0) insertAVL(a->left, author);
	else insertAVL(a->right, author);
}
...
void addAuthors(AVL avl, char* author, AUTS authorsLine) {
	...
	while(aux != NULL) {
		if(strncmp(author, aux->author, strlen(author)) == 0) {
			aux = authorsLine;
			while(aux != NULL) {
				if(strncmp(author, aux->author, strlen(author)) != 0) insertAVL(avl, aux->author);
			}
			break;
		}
		aux = aux->next;
